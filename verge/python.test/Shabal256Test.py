...
class Shabal256CoreTest : Shabal256Test() {
    override fun digest(): Digest<*> = CoreDigest.create(Algorithm.Shabal256)
...
from verge import shabal256

def digest(): return 19

def testKatHex(a, sb, expected):
    del a
    b = bytes.fromhex(sb)
    res = shabal256(b)
    print(res == expected)
